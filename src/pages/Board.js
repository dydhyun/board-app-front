...
const Board = () => {
    ...
    const findById = useCallback(async () => {
        try {
            const response = await axios.get(`http://211.188.52.183:9090/boards/${id}`, {
                headers: {
    ...
    const deleteById = useCallback(async () => {
        try {
            const resonse = await axios.delete(`http://211.188.52.183:9090/boards/${id}`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
    ...
    const modify = useCallback(async (formData) => {
        try {
            const response = await axios.patch('http://211.188.52.183:9090/boards', formData, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
